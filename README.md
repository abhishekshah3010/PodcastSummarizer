...
![](RackMultipart20221203-1-r0h81h_html_b7eae5380e0bafa9.png)
...
![](RackMultipart20221203-1-r0h81h_html_c04a211ca4a7cc3e.png)
...
![](RackMultipart20221203-1-r0h81h_html_abe2db92e7f249b8.png)
...
![](RackMultipart20221203-1-r0h81h_html_35cbdc87a15a8c8a.png) **Figure 3** : Overall and monthly cost

**KEY QUESTIONS**

1. **Lessons learned along the way?**

- Look at resources you have first before figuring out if anything extra is required. Used Free Tier without checking if academy account works for us. Lesson - Try out the available resources before buying anything new.
- Last minute change to the account we used because t2.small was not sufficient and thus caused a sudden change. Should have tested the code in batches. We had created the Lambda functions over a month ago with the required Terraform script but never checked running it on an EC2 as the submission requires. Lesson - Keep testing the code while building the system.

1. **Why is the cloud better for the project you picked?**

- **No storage constraints** - Our application can be extended to handle lengthy audio files since our storage is on the cloud and expanding the storage limits of our AWS S3 would be feasible and can be increased in no time. The front end runs on EC2, hence, we can set up auto-scaling and repetition to increase resiliency and downtime.
- **Anywhere and anytime access** - You can access your application anytime and anywhere via a web browser from any device.
- **Easy Installation**
- **Cost Effective**

1. **Any issues/challenges with the AWS technologies you selected?**

- **Amplify and Cognito** - Initially, we planned to deploy our front-end application on AWS Amplify (free-tier account). Even though Amplify is easy to set up through Command Line Interface (CLI), it does require Amplify Cognito for its authentication. Replicating this on Terraform was particularly challenging since there wasn't enough documentation to highlight how AWS Cognito can be integrated into Amplify. After spending a considerable amount of time, the Flask framework was used for our front-end implementation.
- **AWS SageMaker and Hugging face** - Hugging face pre-trained models were unable to load in AWS SageMaker because of the PyTorch/Tensorflow installation problem. We tried multiple times by installing both PyTorch and Tensorflow using the normal pip installation as well as using homebrew but were still unable to run and load the required model.
- **Background Image -** Initially, we ran into issues with respect to the podcast background image in the EC2 instance for our front-end application. We used the SCP command and key file for copying our background image to EC2.
- **EC2 on Free Tier Account -** Terraform requires the files containing Lambda function code to be in a zip format. This zip is later unzipped by Terraform to create the Lambda functions. We used a t2.small EC2 instance to run the Terraform script. This script gave us a error because the space provisioned for this was not sufficient. We thus had to increase the size of the EC2 instance which isnt free tier eligible. Consecutively, we move the application to the Academy account. This enabled us to use a t2.large EC2 instance without added cost.

1. **What other features or enhancements or would you like to see added to your project if you had more time?**

- **Login -** A login functionality can be added to serve each user and provide personalization to their interests and likes.

- **Scaling -** We could have scaled our application to handle thousands of users.

- **File Formats -** Our application can be extended to handle multiple file formats like MP4, WAV, MPEG-4, etc
- **SNS -** Since our application erases the summarized text after each run without preserving the data. We could add the option to allow the user to input their email address, and the application can send the summary to them.
- **Upload history -** We can also introduce upload history so that users can keep track of all podcasts they previously uploaded to read the summary.
- **Custom ML model -** One of the drawbacks of converting audio to text that is using AWS transcribe is that it is unable to detect text when multiple speakers are speaking simultaneously or when there's loud background introduction music and a speaker is trying to speak something. We could design and implement a custom Machine Learning model to filter out noise and to detect speaker voice clearly for better conversion of audio to text. This will allow us to get better results for summarized text.
- **Translation Service -** To increase the accessibility of our application, we can provide translation for the summarized output using AWS Translate. Users can choose which language the summarized output is written in.
